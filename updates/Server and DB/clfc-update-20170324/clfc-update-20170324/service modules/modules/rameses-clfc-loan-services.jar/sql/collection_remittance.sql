...
SELECT c.objid AS remittanceid, c.collector_name AS collector, c.txndate, c.cbsno, c.totalamount AS amount,
	...
	IFNULL((SELECT ABS(SUM(amount)) FROM collection_remittance_other WHERE parentid = c.objid AND txntype = 'SHORTAGE'), 0) AS shortage,
	IFNULL((SELECT SUM(amount) FROM collection_remittance_other WHERE parentid = c.objid AND txntype = 'OVERAGE'), 0) AS overage,
	IFNULL((SELECT SUM(amount) FROM collection_cb_detail WHERE parentid = cb.objid), 0) AS totalcash,
	IFNULL((
		SELECT SUM(q.amount)
		FROM (
			SELECT d.*
			FROM collection_noncash cn
			INNER JOIN collection_remittance_detail d ON cn.refid = d.refid
			UNION
			SELECT d.*
			FROM collection_remittance_detail d
			WHERE d.payoption = 'check'
		) q
		WHERE q.parentid = c.objid
	), 0) AS totalnoncash
FROM collection_remittance c
INNER JOIN loanpayment p ON c.objid = p.objid
LEFT JOIN loan_route r ON c.group_objid = r.code
LEFT JOIN collection_cb cb ON c.cbsno = cb.cbsno
WHERE c.state = 'POSTED'
	AND c.txndate BETWEEN $P{startdate} AND $P{enddate}
ORDER BY route, c.txndate

[xgetPostedCollectionsByStartdateAndEnddate]
