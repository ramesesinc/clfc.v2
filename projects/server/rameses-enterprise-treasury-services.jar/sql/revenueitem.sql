...
SELECT r.* FROM itemaccount r 
INNER JOIN collectiontype_account ca ON ca.account_objid=r.objid
WHERE ca.collectiontypeid=$P{collectiontypeid} AND (r.title LIKE $P{title}  OR r.code LIKE $P{code} ) 
	${filter}
ORDER BY r.title
