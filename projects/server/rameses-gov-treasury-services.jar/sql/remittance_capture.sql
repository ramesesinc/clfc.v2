...
from draft_remittance drem 
	...
	inner join batchcapture_collection_entry bce on dremc.objid=bce.objid 
	inner join cashreceipt cr on bce.objid = cr.objid 
	inner join batchcapture_collection bc on bce.parentid = bc.objid 
where drem.collector_objid=$P{collectorid} and drem.remittancedate=$P{remittancedate} 
	and bc.state = 'POSTED' and cr.state = 'CAPTURED'  
...
from draft_remittance drem 
	inner join draft_remittance_cashreceipt dremc on drem.objid=dremc.parentid 
	inner join batchcapture_collection_entry bce on dremc.objid=bce.objid 
	inner join cashreceiptpayment_noncash crp ON bce.objid=crp.receiptid 	
	inner join cashreceipt cr on bce.objid=cr.objid 
	inner join batchcapture_collection bc on bce.parentid=bc.objid 
where drem.collector_objid=$P{collectorid} and drem.remittancedate=$P{remittancedate} 
	and bc.state = 'POSTED' and cr.state = 'CAPTURED'  
...
[postRemittedReceipts]
update cashreceipt set state='POSTED' where objid in (
